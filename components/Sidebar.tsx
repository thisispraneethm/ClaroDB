...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  const linkClasses = "flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 group";
  const activeClasses = "bg-primary text-primary-foreground font-semibold shadow-medium shadow-primary/30";
  ...
  const sidebarContent = (
    <>
      <div className="h-20 flex items-center px-4 shrink-0">
        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-primary to-[#5E56F8] flex items-center justify-center text-white shadow-lg shadow-primary/20">
  ...
  return (
    <>
       ...
       <div
        ...
        aria-hidden="true"
      />
      <aside className={`w-64 bg-card/70 backdrop-blur-2xl border-r border-border flex flex-col flex-shrink-0 transition-transform duration-300 ease-in-out z-50
