...
const HomePage: React.FC = () => {
  ...
  const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; description: string; to: string; linkText: string }> = ({ icon, title, description, to, linkText }) => (
    <div className="bg-card/80 backdrop-blur-2xl border border-border rounded-xl p-6 shadow-medium transition-all duration-300 flex flex-col group hover:shadow-large hover:-translate-y-1.5">
        <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-primary/10 text-primary flex items-center justify-center mb-4 transition-all duration-300 group-hover:bg-primary group-hover:text-white group-hover:scale-110 group-hover:rotate-6">
            {icon}
        </div>
    ...
    <div className="h-full overflow-y-auto p-6 md:p-8 lg:p-12 animate-fade-in-up">
      <div className="max-w-6xl mx-auto">
        <div className="text-center pt-16 pb-20">
          ...
          <p className="text-lg md:text-xl text-text-secondary max-w-3xl mx-auto mb-10 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
          ...
          <NavLink 
            to="/demo" 
            className="inline-flex items-center justify-center px-8 py-4 bg-primary text-primary-foreground font-semibold rounded-lg shadow-large shadow-primary/30 hover:bg-primary-hover hover:scale-105 transition-all duration-200 animate-fade-in-up"
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up" style={{ animationDelay: '500ms' }}>
            <FeatureCard
                icon={<FileUp size={24} />}
